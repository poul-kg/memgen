...
internal/
  ...
  knowledge/                 -- File store (store.go) and per-ticket locks (lock.go)
  ticket/                    -- Branch-to-JIRA-ticket extraction
  sources/                   -- JIRA REST API client + GitHub gh CLI wrapper
  claude/                    -- Claude CLI wrapper for summarization/merging
...
- **Storage**: Markdown files in `~/.config/memgen/knowledge/<org>/<repo>/<TICKET>.md`
- **Config**: TOML at `~/.config/memgen/config.toml`
- **External deps**: `claude` CLI (Opus 4.6) for summarization, `gh` CLI for GitHub data, JIRA REST API v3 for ticket data.
...
- External commands (`gh`, `claude`) are abstracted behind `CommandExecutor` interfaces for testability.
...
- `claude.CommandExecutor` — for `claude` CLI calls (has `ExecuteWithStdin`)
...
|------|------|-------------|
| `memgen__init` | `branch` | Gathers JIRA + GitHub data, summarizes via Claude, creates knowledge file |
| `memgen__get` | `branch` | Reads knowledge file, warns if stale |
| `memgen__set` | `branch`, `decisions` | Merges agent decisions into knowledge file via Claude |
| `memgen__refresh` | `branch` | Fetches only new data since last refresh, merges via Claude |
